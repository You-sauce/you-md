...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("YOU-MD~", '')
...
┆ 📞 𝙳𝙴𝚅 𝙽𝙾 : 56945031186
╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄◈`;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/rzbd7d.jpg` }, caption: up })

          const channelJid = "120363404137900781@newsletter"
          try {
...
setInterval(async () => {
    if (config.AUTO_BIO === "true") {
        ...
        const bioText = `❤️ ʏᴏᴜ ᴍᴅ ʙᴏᴛ 🤖 ɪs ʟɪᴠᴇ ɴᴏᴡ\n📅 ${date}\n⏰ ${time}`;
        try {
...
conn?.ev?.on('messages.update', async updates => {
  ...
  const udp = botNumber.split('@')[0];
    const rav = ('56945031186', '56945031186');
